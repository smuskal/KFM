...
a different RDKit than the forests were fitted on produces different features,
scores worse, and nothing in a bundle notices. Asserting "we both use 2025.09.5" is not
proof; this is.

    python -m kfm.rdkit_check --write    once, to record the reference
    python -m kfm.rdkit_check            anywhere else, to verify
...
def features():
    """The encoder, reached the way the tools reach it.

    kfm.extend.ligand_matrix is the copy the extend and buildnew paths use, and
    it is the same recipe the released bundles were fitted with.
    """
    from kfm.extend import ligand_matrix
    return np.asarray(ligand_matrix(SMILES), dtype=np.float32)
    ...
    if a.write:
        json.dump({
            "what": "SHA256 of the float32 (%d, 1038) ligand feature matrix the "
                    "model is fed, for the %d SMILES in kfm/rdkit_check.py"
            ...
            "how_to_verify": "python -m kfm.rdkit_check",
